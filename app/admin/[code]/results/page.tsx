...
import { innspill, items, responses, sessions } from '@/db/schema';
...
type ResultsPageProps = { params: { code: string } };
...
export default async function ResultsPage({ params }: ResultsPageProps) {
  ...
  const [session] = await db
    .select({ id: sessions.id, code: sessions.code, title: sessions.title, mode: sessions.mode, phase: sessions.phase, status: sessions.status, tags: sessions.tags, createdAt: sessions.createdAt })
    .from(sessions)
  ...
  const numericResponses = allResponses.filter((entry) => { const vote = Number(entry.value); return Number.isInteger(vote) && vote >= 1 && vote <= 5; });
  const mappingResponses = allResponses.filter((entry) => { const vote = Number(entry.value); return !Number.isInteger(vote) || vote < 1 || vote > 5; });
  const hasKartlegging = session.mode === 'kartlegging';
  const hasStemming = numericResponses.length > 0;

  const mappingParticipantCount = Math.max(new Set(mappingResponses.map((entry) => entry.participantId)).size, new Set(allResponses.map((entry) => entry.participantId)).size);
  ...
  const voteItems = includedItems.map((item) => {
  ...
  const innspillQuestions = allItems.filter((item) => item.isQuestion).map((question) => ({
    id: question.id,
    text: question.text,
    innspill: innspillRows.filter((entry) => entry.questionId === question.id),
  }));
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-10 text-slate-900 sm:px-6">
      <div className="mx-auto w-full max-w-4xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Link href={`/admin/${session.code}`} className="inline-flex text-sm font-medium text-slate-700 underline print:hidden">← Tilbake til admin</Link>
          {session.mode !== 'aapne-innspill' ? (
            <ExportButton session={{ title: session.title, code: session.code, mode: session.mode, phase: session.phase, created_at: session.createdAt }} items={allItems.map((item) => ({ id: item.id, text: item.text, is_new: item.isNew, created_by: item.createdBy, excluded: item.excluded }))} responses={allResponses.map((entry) => ({ item_id: entry.itemId, participant_id: entry.participantId, value: entry.value }))} tags={session.tags} />
          ) : null}
        </div>

        <header className="rounded-xl border border-slate-200 bg-white p-6"><h1 className="text-3xl font-semibold tracking-tight">{session.title} — Resultater</h1><p className="mt-2 text-sm text-slate-600">Sesjonskode: {session.code}</p></header>

        {session.mode === 'aapne-innspill' ? <InnspillResults questions={innspillQuestions} /> : null}
